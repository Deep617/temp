...
import '../../../dashboard/profile/domain/repositories/profile_repository.dart';
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  bool get _canProceed {
    switch (_page) {
      case 0: return _selectedActivity != null;
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(
          child: PageView(
            ...
            children: [
              _ActivityPage(selected: _selectedActivity, onSelect: (a) => setState(() => _selectedActivity = a)),
              _LevelPage(selected: _selectedLevel, onSelect: (l) => setState(() => _selectedLevel = l)),
              _GoalsPage(selected: _selectedGoals, onToggle: (g) => setState(() => _selectedGoals.contains(g) ? _selectedGoals.remove(g) : _selectedGoals.add(g)), primaryActivity: _selectedActivity),
              _GenderPage(selected: _selectedGender, onSelect: (g) => setState(() => _selectedGender = g)),
        ...
        Padding(
          ...
          child: Column(children: [
            PrimaryButton(
              label:     _page == _totalPages - 1 ? 'Start Matching!' : 'Continue',
              loading:   _saving,
            ...
            if (_page < _totalPages - 1) ...[
              ...
              TextButton(onPressed: _next, child: Text('Skip for now', style: AppTextStyles.bodySM())),
            ],
          ]),
        ),
      ])),
...
class _ActivityPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('What\'s your\nfavourite activity?', style: AppTextStyles.h1()).animate().fadeIn(),
      const SizedBox(height: 6),
      Text('We\'ll match you with people who share your passion', style: AppTextStyles.body()).animate(delay: 100.ms).fadeIn(),
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: isSelected ? color.withOpacity(0.5) : AppColors.border2,
...
class _LevelPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.fromLTRB(28, 24, 28, 0),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Your experience\nlevel?', style: AppTextStyles.h1()).animate().fadeIn(),
      const SizedBox(height: 28),
      ...AppConstants.levels.asMap().entries.map((e) {
        final l = e.value; final isSelected = selected == l['id'];
        return Padding(padding: const EdgeInsets.only(bottom: 12),
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isSelected ? AppColors.primary.withOpacity(0.12) : AppColors.surface2,
                borderRadius: BorderRadius.circular(14),
                border: Border.all(color: isSelected ? AppColors.primary.withOpacity(0.4) : AppColors.border2, width: isSelected ? 1.5 : 1),
              ),
              child: Row(children: [
                Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(color: isSelected ? AppColors.primary.withOpacity(0.2) : AppColors.surface3, borderRadius: BorderRadius.circular(10)),
                  child: Center(child: Text('\${e.key + 1}', style: AppTextStyles.h3(color: isSelected ? AppColors.primary : AppColors.textMuted))),
                ),
                const SizedBox(width: 14),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(l['label']!, style: AppTextStyles.subtitle(color: isSelected ? AppColors.primary : AppColors.textPrimary)),
                  Text(l['desc']!,  style: AppTextStyles.bodySM()),
                ])),
...
class _GoalsPage extends StatelessWidget {
  const _GoalsPage({required this.selected, required this.onToggle, required this.primaryActivity});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: const EdgeInsets.fromLTRB(28, 24, 28, 0),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Your fitness\ngoals?', style: AppTextStyles.h1()).animate().fadeIn(),
        const SizedBox(height: 20),
        Expanded(child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Text(g['label']!,
                            style: AppTextStyles.bodySM(
                                color: isSel ? AppColors.teal : AppColors.textSecondary)
                                .copyWith(fontWeight: FontWeight.w600)),
                      ]),
            ...
            // All goals section
            Text('ALL GOALS', style: AppTextStyles.label(color: AppColors.textMuted)),
            const SizedBox(height: 10),
            Wrap(
              spacing: 8, runSpacing: 8,
              children: allOther.asMap().entries.map((e) {
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: isSel ? AppColors.primary.withOpacity(0.15) : AppColors.surface2,
                      borderRadius: BorderRadius.circular(100),
                      border: Border.all(
                          color: isSel ? AppColors.primary.withOpacity(0.4) : AppColors.border2),
                    ),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Text(g['emoji']!, style: const TextStyle(fontSize: 14)),
                      const SizedBox(width: 6),
...
class _GenderPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...[{'id':'male','label':'Male','emoji':'👨'},{'id':'female','label':'Female','emoji':'👩'},{'id':'other','label':'Other','emoji':'🧑'}]
          .asMap().entries.map((e) {
        final g = e.value; final isSelected = selected == g['id'];
          ...
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              child: Row(children: [
                ...
                Text(g['label']!, style: AppTextStyles.subtitle(color: isSelected ? AppColors.primary : AppColors.textPrimary)),
                const Spacer(),
                if (isSelected) const Icon(Icons.check_circle, color: AppColors.primary),
              ]),
            ).animate(delay: Duration(milliseconds: e.key * 100)).fadeIn().slideX(begin: 0.2),
          ),
        );
      }),
    ]),
...
class _LocationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.fromLTRB(28, 24, 28, 0),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Where are you\nbased?', style: AppTextStyles.h1()).animate().fadeIn(),
      const SizedBox(height: 40),
      AppInput(label: 'City', hint: 'e.g. Mumbai, Delhi, Bangalore', controller: controller,
          prefixIcon: Icons.location_on_outlined, textCapitalization: TextCapitalization.words)
